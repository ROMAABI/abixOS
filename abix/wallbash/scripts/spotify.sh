...
notify_and_set_permissions() {
    local path=$1
    notify-send -a "abix Alert" "Permission needed for Wallbash Spotify theme"
...
configure_spicetify() {
    ...
    if [ ! -d "${spicetify_themes_dir}/Sleek" ]; then
        curl -L -o "${cache_dir}/landing/Spotify_Sleek.tar.gz" "https://github.com/abix-Project/abix/raw/master/Source/arcs/Spotify_Sleek.tar.gz"
