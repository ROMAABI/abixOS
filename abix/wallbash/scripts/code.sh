...
# shellcheck source=$HOME/.local/bin/abix-shell
# shellcheck disable=SC1091
if ! source "$(which abix-shell)"; then
    echo "[wallbash] code :: Error: abix-shell not found."
    echo "[wallbash] code :: Is abix installed?"
    exit 1
fi

cacheDir="${cacheDir:-$XDG_CACHE_HOME/abix}"
...
for i in "${!codeVsix[@]}"; do
    ...
    file=$(find -L "${codeVsix[i]}" -type f -path "*extensions/theabixproject*" -name "wallbash.json")
    if [ -z "${file}" ]; then
        [ -f "${cacheDir}/landing/Code_Wallbash.vsix" ] || curl -L -o "${cacheDir}/landing/Code_Wallbash.vsix" https://github.com/abix-Project/code-wallbash/raw/refs/heads/master/release/Code_Wallbash.vsix
        case ${codeVsix[i]} in
        *".cursor"*)
...
for i in "${!codeConf[@]}"; do
    ...
    extTheme="$(
        "$LIB_DIR/abix/parse.json.py" -CRQ '.["workbench.colorTheme"]' "${codeConf[i]}/User/settings.json"
    )"
    if [ "${extTheme}" != "${set_theme}" ]; then
        "$LIB_DIR/abix/parse.json.py" -CU '.["workbench.colorTheme"]' "${set_theme}" "${codeConf[i]}/User/settings.json"
